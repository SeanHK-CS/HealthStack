...
## v1.6 — Full E2E verification + quiet-luxury theme (this session)
- Verified every feature end-to-end in real Chromium (48 checks): browse rows/arrows/see-all, group filters + feedback counts, search + clear-filters, detail view, save/unsave + badges, supplements (sort/filters/links/stack), calculator (US/metric/error), protein cross-link, food search, today card (shuffle/split override/add-all), plans (notes/reorder/remove/share), share round-trip in a fresh page, tampered links, import, coach chat (workout/supplement/protein/another/detail-tap/fallback), last-tab restore, 390/320 overflow.
- FIXED (found by the sweep): importing a shared plan in a fresh browser left the builder showing its stale empty render; dismissing the shared view now re-renders the builder. Smoke test strengthened to clear the draft before importing.
